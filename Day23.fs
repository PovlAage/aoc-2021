...
let shortestPath connections initial =
    let unvisited = System.Collections.Generic.PriorityQueue<State, int>()
    let tentative = System.Collections.Generic.Dictionary<State, int>()
    let visited = System.Collections.Generic.HashSet<State>()
    let rec loop (current:State) =
        let tentCurrent = tentative[current]
        match eval current with
        | Done -> tentCurrent
        | NotDone ->
            let moves = enumeratemoves connections current
            let neighbours = moves |> Seq.map (fun m -> (executeMove current m, cost m))
            let tryGet s =
                let mutable v = -1
                if tentative.TryGetValue(s, &v) then
                    Some v
                else
                    None

            let improved = neighbours |>
                            Seq.map (fun (s, c) -> (s, c, tentCurrent+c, tryGet s)) |>
                            Seq.filter (fun (s, c, t, e) -> e.IsNone || e.Value > t) |>
                            Seq.map (fun (s, c, t, e) -> (s, t)) |>
                            List.ofSeq
            let folder tentative (s:State, t) = tentative |> Map.add s t
            for (s, t) in improved do
                tentative[s] <- t
            visited.Add current |> ignore
            for (state, t) in improved do
                unvisited.Enqueue(state, t)
            let mutable next = unvisited.Dequeue()
            while (visited.Contains next) do
                next <- unvisited.Dequeue()
            loop next

    unvisited.Enqueue(initial, 0)
    tentative.Add(initial, 0)
    loop initial

let calcA = shortestPath (connections 2)
let calcB = shortestPath (connections 4)
